...
generate_bfplot(list(
  exp = study13_priming.4$expbf,
  con = study13_priming.4$conbf
))
```
...
generate_rwplot(list(
  exp = study13_priming.4$sumsexp,
  con = study13_priming.4$sumscon
), 20)
...
### Exp
```{r}
generate_coeplot(study13_priming.4$expbf)
```
### Con
```{r}
generate_coeplot(study13_priming.4$conbf)
```
:::
