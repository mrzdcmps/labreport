...
generate_bfplot(list(
  exp = study15_smokers.2$BF.exp,
  con = study15_smokers.2$BF.con
))
```
...
generate_rwplot(list(
  exp = study15_smokers.2$sum_condition_experimental,
  con = study15_smokers.2$sum_condition_control
), 20)
...
### Exp
```{r}
generate_coeplot(study15_smokers.2$BF.exp)
```
### Con
```{r}
generate_coeplot(study15_smokers.2$BF.con)
```
:::
