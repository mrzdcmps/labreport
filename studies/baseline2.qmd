...
## Bayes Factor
```{r}
generate_bfplot(study27_baseline.2$BF)
```

## Random Walk
```{r}
rw <- 20-cumsum(study27_baseline.2$hits - 12)
changeofevidence::plotrw(rw, n_bits = 20)
```

## Change of Evidence
```{r}
generate_coeplot(study27_baseline.2$BF)
```
:::
