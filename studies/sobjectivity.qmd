...
generate_table("Sobjectivity [12]")
```
...
generate_bfplot(list(
  `Study 1` = study16_sobjectivity.1$osave.bf,
  `Study 2` = study17_sobjectivity.2$osum1.bf
))
```
...
generate_rwplot(list(
  `Study 1` = study16_sobjectivity.1$sums,
  `Study 2` = study17_sobjectivity.2$sums
), 20)
...
### Study 1
```{r}
generate_coeplot(study16_sobjectivity.1$osave.bf)
```
### Study 2
```{r}
generate_coeplot(study17_sobjectivity.2$osum1.bf)
```
:::
