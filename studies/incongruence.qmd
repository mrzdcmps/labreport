...
generate_bfplot(list(
  high = study9_incongruence.h$BF,
  low = study9_incongruence.l$BF
))
```
...
generate_rwplot(list(
  high = study9_incongruence.h$self,
  low = study9_incongruence.l$self
), 10)
...
### High
```{r}
generate_coeplot(study9_incongruence.h$BF)
```
### Low
```{r}
generate_coeplot(study9_incongruence.l$BF)
```
:::
