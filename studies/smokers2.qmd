...
generate_table("Smokers 2")
```

## Hypothesis

In this replication study ([Smokers 1](smokers1.html)), we used cigarette addiction as unconscious drive. We hypothesized that regular cigarette smokers would influence the outcome of a quantum RNG that determined whether the participant was going to see a smoking-related or a neutral picture. We predicted that smokers would show less smoking-related stimuli than expected by chance, matching their motivational state of deficiency with regard to smoking or nicotine.
...
generate_demographics_table(
  data = subset(study10_smokers, Study==2),
  age_col = "Age",
  gender_col = "Sex",
  male_value = "2",
  female_value = "1"
...
generate_bfplot(list(
  exp = study10_smokers.2.exp$BF,
  con = study10_smokers.2.con$BF
))
```
...
generate_rwplot(list(
  exp = study10_smokers.2.exp$PosBilder,
  con = study10_smokers.2.con$PosBilder
), 400)
...
### Exp
```{r}
generate_coeplot(study10_smokers.2.exp$BF)
```
### Con
```{r}
generate_coeplot(study10_smokers.2.con$BF)
```
:::

::: {.callout-tip title="Data" collapse="false"}
-   [Download Data](../data/smokers/smokers2.csv)
-   [Download Data "Smokers"](../smokers/smokers2_exp.csv)
-   [Download Data "Non-Smokers"](../data/smokers/smokers2_con.csv)
:::
...
-   [Preregistration](https://osf.io/4fzq8)
