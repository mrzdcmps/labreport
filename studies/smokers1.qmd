...
generate_demographics_table(
  data = subset(study10_smokers, Study==1),
  age_col = "Age",
  gender_col = "Sex",
  male_value = "2",
  female_value = "1"
...
generate_bfplot(list(
  exp = study10_smokers.1.exp$BF,
  con = study10_smokers.1.con$BF
))
```
...
generate_rwplot(list(
  exp = study10_smokers.1.exp$PosBilder,
  con = study10_smokers.1.con$PosBilder
), 400)
...
### Exp
```{r}
generate_coeplot(study10_smokers.1.exp$BF)
```
### Con
```{r}
generate_coeplot(study10_smokers.1.con$BF)
```
:::
