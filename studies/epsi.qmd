...
generate_bfplot(list(
  "Condition A" = study19_epsi$BF.exp,
  "Condition B" = study19_epsi$BF.con
))
```
...
generate_rwplot(list(
  high = study19_epsi$sumsexp,
  low = study19_epsi$sumscon
), 20)
...
### Condition A
```{r}
generate_coeplot(study19_epsi$BF.exp)
```
### Condition B
```{r}
generate_coeplot(study19_epsi$BF.con)
```
:::
