...
generate_bfplot(list(
  lucky = study26_baseline.1.lucky$BF,
  unlucky = study26_baseline.1.unlucky$BF
  ))
```

## Random Walk
```{r}
rw1 <- 20-cumsum(study26_baseline.1.lucky$hits - 12)
rw2 <- cumsum(study26_baseline.1.unlucky$hits - 8)
changeofevidence::plotrw(list(lucky = rw1, unlucky = rw2), n_bits = 20)
```

## Change of Evidence
### Lucky
```{r}
generate_coeplot(study26_baseline.1.lucky$BF)
```
### Unlucky
```{r}
generate_coeplot(study26_baseline.1.unlucky$BF)
```
:::
