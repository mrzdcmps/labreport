...
generate_bfplot(list(
  exp = study13_priming.1$expbf,
  con = study13_priming.1$conbf
))
```
...
generate_rwplot(list(
  exp = study13_priming.1$sumsexp,
  con = study13_priming.1$sumscon
), 20)
...
### Exp
```{r}
generate_coeplot(study13_priming.1$expbf)
```
### Con
```{r}
generate_coeplot(study13_priming.1$conbf)
```
:::
